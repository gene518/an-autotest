...
**File:** `transfer_to_doctor/otc_to_doctor.spec.ts`

**测试目标:** 验证用户购买非处方药时AI自动转接人工医生的完整流程

**实现细节:**

**步骤1: 登录并跳转到IM页面（合并链接）**
- 访问合并URL：`https://www.jk.cn/landing_smspassword?appId=1&mobile=16013156866&needRedirect=true&smsPassword=666666&returnUrl=https%3A%2F%2Fwww.jk.cn%2Fturky%2F%3FisImmsersive%3Dtrue%26native_back_button_color%3D0%26update_native_backbtn%3Dtrue%23%2Fim%2F10407%3FaccessEntrance%3DAIKYSRHD%26oneKey%3D1`
- 登录成功后自动跳转到IM页面
- **断言:** 使用`page.getByText('对话', { exact: true }).first()`验证'对话'标签可见
- **断言:** 验证'我的'标签可见
- **断言:** 验证聊天输入框`page.getByRole('textbox', { name: '有问题尽管问我...' })`可见

**步骤3: 开启新对话**
- 使用`page.getByRole('button').nth(1)`点击右上角第2个按钮（新对话图标）
- **目的:** 清空之前的对话历史，开始全新对话

**步骤4: 输入购药需求**
- 点击输入框：`page.getByRole('textbox', { name: '有问题尽管问我' })`
- 填写文本："我要买红霉素软膏"
- **技术细节:** 使用`fill()`方法填充输入框

**步骤5: 发送消息（移动端触摸交互）**
- 定位发送按钮：`page.locator('span.chat-send-btn')`
- 等待按钮可见：`waitFor({ state: 'visible' })`
- **关键:** 使用`tap()`而非`click()`模拟移动端触摸点击
- 等待10秒让AI完全处理和回复

**步骤6: 验证AI药品介绍**
- **断言:** 使用`.first()`验证对话中显示"红霉素软膏"文本
...
**步骤8: 验证转医生提示**
- **断言:** 验证显示"转至在线医生"文本
- **业务含义:** AI判断需要医生咨询，提示即将转接

**步骤9: 验证科室分配信息**
- **断言:** 使用正则`/【.*科】/`验证显示科室信息（如"【皮肤科】"）
- **业务含义:** 系统已根据药品类型分配相应科室

**步骤10: 验证医生接入**
- **断言:** 使用正则`/什么症状|可以帮助|您好|历史聊天记录/`验证医生发送的第一条消息
- **业务含义:** 人工医生已成功接入对话
...
- 访问合并URL：`https://www.jk.cn/landing_smspassword?appId=1&mobile=16013156866&needRedirect=true&smsPassword=666666&returnUrl=https%3A%2F%2Fwww.jk.cn%2Fturky%2F%3FisImmsersive%3Dtrue%26native_back_button_color%3D0%26update_native_backbtn%3Dtrue%23%2Fim%2F10407%3FaccessEntrance%3DAIKYSRHD%26oneKey%3D1`
- 登录成功后自动跳转到IM页面
- **断言:** 验证'对话'标签（使用`exact: true`精确匹配）
- **断言:** 验证'我的'标签
- **断言:** 验证输入框可见
...
**步骤6: 验证消息发送成功**
- **断言:** 验证输入框被清空（`inputValue()`返回空字符串）
- **断言:** 验证"转人工"消息显示在对话中

**步骤7: 验证AI回复**
- **断言:** 验证AI回复包含"我是您的医生朋友，医疗健康相关问题可以咨询我"
- **实际行为:** 在当前实现中，AI并未立即转至人工医生，而是回复标准话术
- 等待1秒观察后续可能的转接
...
**测试配置:** 
- 测试超时时间：5分钟（300000ms）
- **原因:** 需要多轮对话才能积累足够积分触发转人工

**测试目标:** 验证通过多次咨询积累积分后自动触发转医生流程
...
**步骤2: 访问IM页面并验证**
- 访问IM页面
- 等待页面domcontentloaded事件
- **断言:** 验证'对话'标签（使用`exact: true`）
- **断言:** 验证'我的'标签
- **断言:** 验证输入框可见（使用正则`/有问题尽管问我/`）

**步骤3: 开启新对话**
- 点击第2个button
- 等待欢迎话术显示
- **断言:** 验证显示以"您好"开头的欢迎语（使用`/^您好/`正则，`.first()`避免多匹配，超时5秒）

**步骤4: 循环发送消息积累积分**
- **循环配置:**
  - 消息内容："硼酸洗剂如何使用"
  - 最大循环次数：15次
  - 发送按钮定位：`page.locator('span.chat-send-btn')`
...
- **每轮循环逻辑:**
  ...
  3. 填写消息：`fill(message)`
  4. 等待发送按钮可见（超时3秒）
  5. **关键:** 使用`tap()`模拟移动端触摸点击
  6. 等待8秒AI回复（给足够时间处理）
  7. 检查是否触发转医生：
     - 使用`page.locator('text=/转至.*科室|转至在线医生|免费为您转至/')`
     - 检查count是否大于0
  8. 如触发则记录轮数并退出循环

- **输出:** 控制台输出`转医生流程在第 ${i} 条消息后触发`

**步骤5: 验证转医生流程已触发**
- **断言:** `expect(transferTriggered).toBe(true)` - 确保在15次内触发

**步骤6: 验证转接提示**
- **断言:** 验证显示"免费为您转至相关科室"
- **业务含义:** 系统判定需要更专业的咨询，免费转接

**步骤7: 验证医生信息**
- **断言:** 使用正则`/主治医师|主任医师|副主任医师/`验证显示医生职称
- 使用`.first()`获取第一个匹配元素
- **业务含义:** 显示即将服务的医生资质

**步骤8: 验证医生接入**
- 等待5秒医生回复
- **断言:** 使用正则`/什么症状|可以帮助|您好/`验证医生消息
- 使用`.nth(1)`获取第2个匹配（第1个可能是AI的）
- 超时15秒
...
**File:** `transfer_to_doctor/send_message.spec.ts`
...
**步骤7: 验证发送成功**
- **断言:** 验证输入框被清空（`toHaveValue('')`）
- **业务含义:** 消息成功发送后输入框会自动清空

**步骤8: 验证消息显示**
- **断言:** 验证发送的消息"硼酸洗剂如何使用"显示在对话中
- **关键:** 使用`.last()`获取最新发送的消息
- **业务含义:** 用户发送的消息应该立即显示在对话界面
